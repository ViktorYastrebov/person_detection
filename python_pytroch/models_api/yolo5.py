...
class YolosV5:
    def __init__(self, model_path):
        import sys
        # it keeps the file structure:
        #      models/yolo.py
        #      models/common.py etc
        pytorch_fix = Path(__file__).resolve().absolute().parent
        # 'd:/viktor_project/person_detection/pedestrian_detection/models_api/yolo'
        sys.path.insert(0, str(pytorch_fix))

        self.device = torch.device('cuda:0')
        self.model = torch.load(model_path, map_location=self.device)['model'].float()  # load to FP32
