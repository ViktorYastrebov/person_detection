...
namespace sort_tracker {
    ...
    namespace helpers {
        ...
        double GetIOU(const DetectionBox &bb_text, const DetectionBox &bb_gt) {

            float bbox_tl_1 = bb_text[0];
            float bbox_tl_2 = bb_text[1];
            float bbox_br_1 = bb_text[0] + bb_text[2];
            float bbox_br_2 = bb_text[1] + bb_text[3];
            float area_bbox = bb_text[2] * bb_text[3];

            float tl_1 = std::max(bbox_tl_1, bb_gt(0));
            float tl_2 = std::max(bbox_tl_2, bb_gt(1));
            float br_1 = std::min(bbox_br_1, bb_gt(0));
            float br_2 = std::min(bbox_br_2, bb_gt(1));

            float w = br_1 - tl_1; w = (w < 0 ? 0 : w);
            float h = br_2 - tl_2; h = (h < 0 ? 0 : h);
            float area_intersection = w * h;
            float area_candidates = bb_gt(2) * bb_gt(3);
            float un = area_bbox + area_candidates - area_intersection;
            if (un < DBL_EPSILON) {
                return 0.0;
            }
            return static_cast<double>(area_intersection / un);
        }
    ...
    TrackerMatch TrackersPool::process_match(const std::vector<DetectionBox> &predicted, const DetectionResults &detections) {
        ...
        for (size_t col = 0; col < detections.size(); col++) {
            ...
            if (!flag) {
                res.unmatched_detections.push_back(col);
            }
        }

        for (size_t row = 0; row < trackers_.size(); row++) {
            bool flag = false;
            for (int i = 0; i < indices.rows(); i++)
                if (indices(i, 0) == row) {
                    flag = true;
            ...
            if (!flag) {
                ...
                res.unmatched_tracks.push_back(row);
            }
        }
        ...
        for (int i = 0; i < indices.rows(); i++) {
            ...
            int track_idx = row;
            int detection_idx = col;
            if (matrix(row, col) < iou_threshold_) {
