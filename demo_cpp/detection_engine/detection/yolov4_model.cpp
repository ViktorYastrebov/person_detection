...
YoloV4::YoloV4(const std::string &path, const std::string &config, const std::vector<int> &classes, const float confidence, RUN_ON device)
...
std::vector<DetectionResult> YoloV4::process(const cv::Mat &frame) {
        ...
        for (const auto &l2 : l1) {
            for (int i = 0; i < l2.rows; ++i) {
                ...
                if (it != filtered_classes_.cend() && *value > conf_threshold_) {
                    ...
                    classes.push_back(static_cast<int>(class_id));
                }
            }
        }
    }
