...
YoloV4Batched::YoloV4Batched(const std::string &path, const std::string &config, const std::vector<int> &classes, const float confidence, RUN_ON device)
...
std::vector<std::vector<DetectionResult>> YoloV4Batched::process(const std::vector<cv::Mat> &frames) {
